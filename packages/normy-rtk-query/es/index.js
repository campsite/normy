...
export var getNormalizer = function getNormalizer(dispatch) {
  return dispatch({
...
var isNormalizerAction = function isNormalizerAction(action) {
  return isAction(action) && allTypes.includes(action.type);
};
export var createNormalizationMiddleware = function createNormalizationMiddleware(api, normalizerConfig) {
  var normalizer = createNormalizer(_extends({}, normalizerConfig, {
